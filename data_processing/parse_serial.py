...
while True:
    ...
    with open(f"demo_data/raw/{sample_class}/sample_{str(i).zfill(3)}.npy", "wb") as f:
        np.save(f, mic_in)

    with wave.open(f'demo_data/{sample_class}/sample_{str(i).zfill(3)}.wav','w') as obj:
        obj.setnchannels(1)
        obj.setsampwidth(2)
